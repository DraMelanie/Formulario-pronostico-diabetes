...
export default function ReversalQuiz() {
  ...
  const canAdvance = () => {
    if (step === 0) return formData.nombre && formData.correo && formData.instagram && formData.whatsapp && formData.país;
  ...
  const getSegmento = useCallback(() => {
    ...
    if (score >= 15) {
      if (formData.presupuesto && formData.presupuesto !== 'NoPodría') {
        ...
        return { score, categoría: 'Alto Pronóstico', etiqueta: 'Prospecto_Formulario_free', mensaje: '🔥 ALTO PRONÓSTICO DE REVERSIÓN', detalle: 'Tu perfil indica excelente potencial. Obtén nuestro ebook gratuito.', cta: 'Descargar Ebook Gratuito', ctaUrl: '#' };
      }
    } else if (score >= 10 && score < 15) {
      if (formData.presupuesto && formData.presupuesto !== 'NoPodría') {
        return { score, categoría: 'Pronóstico Intermedio', etiqueta: 'Prospecto_Formulario_VSL', mensaje: '⚠️ PRONÓSTICO INTERMEDIO', detalle: 'Tu perfil muestra potencial moderado. Podemos ayudarte a mejorarlo.', cta: 'Ver Metodología de Reversión', ctaUrl: 'https://www.somosplantpowered.com/vsl-piad' };
      } else {
        return { score, categoría: 'Pronóstico Intermedio', etiqueta: 'Prospecto_Formulario_free', mensaje: '⚠️ PRONÓSTICO INTERMEDIO', detalle: 'Tu perfil muestra potencial moderado. Accede a nuestro ebook.', cta: 'Ebook Gratuito + Training "Domina tu Glucosa" (10 días)', ctaUrl: '#' };
      }
    } else {
